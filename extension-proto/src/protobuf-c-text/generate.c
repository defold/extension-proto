      ...
      case PROTOBUF_C_TYPE_STRING:
        if (f[i].label == PROTOBUF_C_LABEL_REPEATED) {
          for (j = 0; j < quantifier_offset; j++) {
            ...
            escaped = (unsigned char*)esc_str(
                (char*)STRUCT_MEMBER(unsigned char **, m, f[i].offset)[j],
                strlen((const char*)STRUCT_MEMBER(unsigned char **, m, f[i].offset)[j]),
                allocator);
            if (!escaped) {
              PBC_FREE(rs->s);
              rs->s = NULL;
            ...
            rs_append(rs, level + strlen(f[i].name) + strlen((const char*)escaped) + 10,
                allocator,
          ...
          escaped = (unsigned char*)esc_str(
              (char*)STRUCT_MEMBER(unsigned char *, m, f[i].offset),
              strlen((const char*)STRUCT_MEMBER(unsigned char *, m, f[i].offset)),
              allocator);
          if (!escaped) {
            PBC_FREE(rs->s);
            rs->s = NULL;
      ...
      case PROTOBUF_C_TYPE_BYTES:
        if (f[i].label == PROTOBUF_C_LABEL_REPEATED) {
          for (j = 0; j < quantifier_offset; j++) {
            ...
            escaped = (unsigned char*)esc_str(
                (char*)STRUCT_MEMBER(ProtobufCBinaryData *, m, f[i].offset)[j].data,
          ...
          escaped = (unsigned char*)esc_str(
              (char*)STRUCT_MEMBER(ProtobufCBinaryData, m, f[i].offset).data,
